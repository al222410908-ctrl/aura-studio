...
export function Metrics() {
  return (
    <section className="bg-glow-emerald border-y border-border/70 bg-surface/40 py-14 sm:py-16">
        ...
        <Reveal className="grid grid-cols-2 gap-6 sm:gap-8 lg:grid-cols-4">
          {metrics.map((m) => (
            <RevealItem key={m.label} className="relative min-w-0 pl-3">
              ...
              <p className="text-3xl font-extrabold tracking-tight text-rust sm:text-4xl">
                {m.value}
              </p>
              <p className="mt-2 text-sm font-semibold text-foreground">{m.label}</p>
              <p className="mt-1 text-xs text-foreground/75">{m.detail}</p>
            </RevealItem>
          ))}
        </Reveal>
        ...
        <div className="relative mt-12 rounded-2xl border border-rust/15 bg-background/40 p-6 backdrop-blur-xl">
          ...
          <ul className="mt-4 flex flex-wrap gap-2.5">
            {techLogos.map((tech) => (
              <li
                key={tech}
                className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/5 px-3 py-1.5 text-xs font-semibold tracking-tight text-foreground/85"
              >
